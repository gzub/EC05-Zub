...
#if defined (CONFIG_TARGET_LOCALE_EUR) ||  defined (CONFIG_ARIES_EUR) || defined(CONFIG_ARIES_VER_B2)
...
/*
 * GT-I9000 CP boot Fail problem
 * Some devices was phone boot fail, cause by AP IO control before CP HW logic initialize(Phone Power On).
 * AP set the PDA Active, nINT_ONEDRAM_CP and RESET_REQ_N pin to low before Phone power on. 
 */
...
static void msm_on(struct modemctl *mc)
{
	...
	gpio_set_value(mc->gpio_phone_on, 0);
	msleep(500);
	gpio_set_value(mc->gpio_phone_pda_active, 1);
        msleep(100);
